...
namespace tt { namespace endpoints {
    ...
    const std::array<std::string, 3> kTradeStatusStr = { "pending", "completed", "failed" };

    class TradeSubmitHandler final : public server::handlers::HttpHandlerJsonBase
    {
    public:
        static constexpr std::string_view kName = "handler-user-trade-submit";

        TradeSubmitHandler(const components::ComponentConfig& config,
        ...
        {
            try {
                auto request_dom = json.As<TradeSubmitRequestBody>();
                auto response_dom = TradeSubmit(request_dom, context);
    ...
    private:
        ...
        void UpdateUserBalanceAndStocks(const std::string& username,
                                        const int64_t balance,
                                        const int64_t stocks) const;

        void UpdateTradeByUUID(const boost::uuids::uuid trade_id,
                               const int64_t net_profit,
                               const std::string& status) const;

        boost::uuids::uuid InsertTradeForUser(const std::string& username,
        ...
        TradeSubmitResponseBody TradeSubmit(
